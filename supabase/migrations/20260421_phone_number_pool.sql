-- Phone number pool for local-presence outbound dialing.
-- Spec originally referenced `profiles`; this project uses `softphone_users` as the profile table.
...
CREATE POLICY "admins manage pool" ON phone_number_pool
  ...
  USING (
    EXISTS (
      SELECT 1 FROM softphone_users
      WHERE softphone_users.id = auth.uid()
        AND softphone_users.role = 'admin'
    )
  )
  WITH CHECK (
    EXISTS (
